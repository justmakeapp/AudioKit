...
//
    ...
    public extension MIDI {
        ...
        func addListener(_ listener: MIDIListener) {
            listeners.append(listener)
        }

        /// Remove a listener
        /// - Parameter listener: MIDI Listener
        func removeListener(_ listener: MIDIListener) {
    ...
    public extension MIDI {
        ...
        func inputName(for inputUid: MIDIUniqueID) -> String? {
        ...
        func uidForInputAtIndex(_ inputIndex: Int = 0) -> MIDIUniqueID {
        ...
        ///
        /// - Parameter inputIndex: Index of source port
        func openInput(index inputIndex: Int) {
        ...
        internal enum UMPMessageType: UInt8 {
        ...
        internal enum UMPSysEx7bitStatus: UInt8 {
        ...
        private func byteArray(from value: some FixedWidthInteger) -> [UInt8] {
        ...
        /// UMP Message Type can tell us how many 32bit UMP packets we have to read next in order to get full MIDI
        /// message.
        ...
        private func processUMPSysExMessage(with bytes: [UInt8]) -> [UInt8] {
            ...
            // We need to use new UMP message type field and add these flags to make existing MIDI parser code happy and
            // people expect to have F0 and F7 in the SysEx message I think
        ...
        private func processUMPMessages(_ midiEventPacket: MIDIEventList.UnsafeSequence.Element) -> [MIDIEvent] {
            ...
            while wordIndex < midiEventPacket.pointee.wordCount {
                ...
                case .Reserved32bit_1, .Reserved32bit_2:
                    Log(
                        "Got unsupported 32 bit UMP message of type: \(String(describing: umpMessageType))",
                        log: OSLog.midi
                    )
                    wordIndex += 1
                ...
                case .MIDI2ChannelVoice64bit, .Reserved64bit_3, .Reserved64bit_4, .Reserved64bit_5:
                    Log(
                        "Got unsupported 64 bit UMP message of type: \(String(describing: umpMessageType))",
                        log: OSLog.midi
                    )
                    wordIndex += 2
                case .Reserved96bit_6, .Reserved96bit_7:
                    Log(
                        "Got unsupported 96 bit UMP message of type: \(String(describing: umpMessageType))",
                        log: OSLog.midi
                    )
                    wordIndex += 3
                case .Data128bit, .Reserved128bit_8, .Reserved128bit_9, .Reserved128bit_10:
                    Log(
                        "Got unsupported 128 bit UMP message of type \(String(describing: umpMessageType))",
                        log: OSLog.midi
                    )
                    wordIndex += 4
                default:
                    ...
                    wordIndex = Int(midiEventPacket.pointee
                        .wordCount) // data probably corrupted, skipping rest of the packet, exiting while loop
                }
            }
        ...
        ///
        func openInput(uid inputUID: MIDIUniqueID) {
                ...
                if inputUID == 0 || inputUID == uid {
                    ...
                    if var port = inputPorts[inputUID] {
                        ...
                        if #available(iOS 14.0, macOS 11.0, *) {
                            ...
                            inputPortCreationResult = MIDIInputPortCreateWithProtocol(client, inputPortName, ._1_0,
                                                                                      &port) { eventPacketList, _ in
                                ...
                                for midiEventPacket in eventPacketList.unsafeSequence() {
                                    ...
                                    for transformedEvent in transformedMIDIEventList
                                        where transformedEvent.status != nil
                            ...
                            inputPortCreationResult = MIDIInputPortCreateWithBlock(client, inputPortName,
                                                                                   &port) { packetList, _ in
        ...
        ///
        /// - Parameter inputIndex: Index of source port
        func closeInput(index inputIndex: Int) {
            let uid = uidForInputAtIndex(inputIndex)
            closeInput(uid: uid)
        }
        ...
        ///
        func closeInput(uid inputUID: MIDIUniqueID) {
