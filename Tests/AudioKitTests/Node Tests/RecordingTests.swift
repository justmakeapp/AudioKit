    ...
    class RecordingTests: AudioFileTestCase {
        func testMultiChannelRecording() throws {
            guard Bundle.main.object(forInfoDictionaryKey: "NSMicrophoneUsageDescription") != nil else {
                Log(
                    "Unsupported test: To record audio, you must include the NSMicrophoneUsageDescription in your Info.plist.",
                    type: .error
                )
                return
            }
            ...
            AVCaptureDevice.requestAccess(for: .audio) { allowed in
                ...
                do {
                    ...
                    try self.recordWithLatency(url: url, channelMap: channelMap, ioLatency: 12_345)
                    expectation.fulfill()
